...
<div class="w-full max-w-md mx-auto p-6 bg-white/80 rounded-3xl shadow-lg backdrop-blur-sm animate-glow">
  @if (result(); as res) {
    ...
    <div class="mb-6">
      ...
      <div class="grid grid-cols-2 gap-4">
        @for (img of res.previewImages; track img) {
          <img [src]="img" (click)="showImage(img)" alt="Пример образа в стиле" class="rounded-xl shadow-md w-full h-auto object-cover aspect-[3/4] cursor-pointer hover:opacity-90 transition-opacity">
        }
      </div>
    </div>

    <!-- Crystal Teaser -->
    <div (click)="goToMonetization()" class="my-6 p-4 bg-lavender/40 rounded-2xl relative overflow-hidden cursor-pointer hover:shadow-lg transition-shadow">
    ...
    <!-- Celebrity Teaser -->
    <div (click)="goToMonetization()" class="my-6 p-4 bg-mint/40 rounded-2xl relative overflow-hidden cursor-pointer hover:shadow-lg transition-shadow">
        <div class="absolute top-2 right-3 text-gold-accent/70">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg>
        </div>
    ...
    <!-- Interior Teaser -->
    <div (click)="goToMonetization()" class="my-6 p-4 bg-beige/80 rounded-2xl relative overflow-hidden cursor-pointer hover:shadow-lg transition-shadow">
      <div class="absolute top-2 right-3 text-gold-accent/70">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" /></svg>
      </div>
      <h3 class="font-serif text-2xl font-semibold text-center">Интерьер твоей души</h3>
      <div class="mt-2 text-center blur-sm select-none">
          <p class="font-sans text-sm">{{ res.interiorTeaser.description }}</p>
          <div class="flex justify-center gap-2 mt-2">
            @for(img of res.interiorTeaser.previewImages; track img) {
              <img [src]="img" alt="Пример интерьера" class="w-16 h-16 object-cover rounded-md">
            }
          </div>
      </div>
      <p class="text-center font-sans mt-2 text-sm">Полный дизайн-проект с примерами комнат — в <span class="font-bold text-gold-accent">полной версии</span>.</p>
    </div>
